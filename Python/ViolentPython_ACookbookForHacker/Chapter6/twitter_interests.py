...
def get_tweets(handle):
    ...
    browser = AnonBrowser()
    browser.anonymize()
    response = browser.open(f'https://twitter.com/'
                            f'search.json?q={query}')
    response = response.text
    response = html_to_json.convert(response)
...
#    for result in json_objects['results']:
    new_result = {
        'from_user': response['_attributes'],
        'geo': response['meta'],
        'tweet': response['link']
    }
...
def find_interests(tweets):
    ...
    for tweet in tweets:
        ...
        interests['users'] += re.compile(f'(@\w+)').findall(text)
