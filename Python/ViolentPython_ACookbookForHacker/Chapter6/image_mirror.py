...
def mirror_images(tgt_url, dest_url):
    ab = AnonBrowser()
    ab.anonymize()
    ab.open(tgt_url)
    ...
    for image in image_tags:
        ...
        filename = os.path.join(dest_url, filename.replace('/', '_'))
#        print(filename)
#        data = ab.open('http://' + image['src']).read()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', type=str, metavar='DESTINATION_DIR',
                        required=True, help='specify the destination directoty')
