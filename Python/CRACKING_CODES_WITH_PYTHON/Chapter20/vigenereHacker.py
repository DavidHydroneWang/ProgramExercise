...
def getMostCommonFactors(seqFactors):
    ...
    for factor in factorCounts:
        ...
        if factor <= MAX_KEY_LENGTH:
            # factorByCount is a list of tuples: (factor, factorCount)
            # factorByCount has a value like: [(3, 497), (2, 487), ...]
...
def kasiskiExamination(ciphertext):
    ...
    # (See getMostCommonFactors() for a description of factorByCount.)
...
def attemptHackWithKeyLength(ciphertext, mostLikelyKeyLength):
    ...
    # allFreqScores is a list of mostLikelyLength number of lists
...
def hackVigenere(ciphertext):
    ...
    for keyLength in allLikelyKeyLengths:
        if not SILENT_MODE:
            print('Attemptiong hack with key length %s (%s possible keys)...' % (keyLength,
    ...
    if hackedMessage is None:
        if not SILENT_MODE:
            print('Unable to hack message with likely length(s). Brute forcing key length...')
