...
contries = ['KOR', 'PAN', 'MEX', 'ENG', 'COL', 'JPN', 'POL', 'SEN',
            'RUN', 'EGY', 'POR', 'MAR', 'URU', 'KSA', 'IRN', 'ESP',
            'DEN', 'AUS', 'FRA', 'PER', 'ARG', 'CRO', 'BRA', 'CRC',
            'NGA', 'ISL', 'SRB', 'SUI', 'BEL', 'TUN', 'GER', 'SWE']

urls = {
    'group': 'http://worldcup.sfg.io/teams/group_results',
    'country': 'http://worldcup.sfg.io/matches/country?fifa_code=',
    'today': 'htttp://worldcup.sfg.io/matches/today',
...
def receive_sms():
    ...
    if body == 'today':
        ...
        for match in html:
            if match['status'] == 'complete':
                output += (
                    ...
                    match['away_team']['counry'] + " " +
                    str(match['away_team']['goal']) + "\n"
                )
        output += "\n\n--- Future Matches ---\n"
        for match in html:
            if match['status'] == 'futute':
                output += (
                    match['home_team']['country'] + " vs " +
                    match['away_team']['country'] + " at " +
                    parser.parse(match['detaime'])
                    .astimezone(to_zone)
        ...
        output = (
            'Sorry we could not understand your response. '
