...
import curses
from datetime import datetime as parsedata_to_datetime


class liginFrom(npyscreen.ActionPopup):
...
class emailList(npyscreen.MultiLine):
    ...
    def handle_selection(self, k):
        ...
        self.parent.parentApp.email_detail_form.date.value = parsedata_to_datetime(
            data['date']
        ).strftime("%a, %d %b")
...
class emailListFrom(npyscreen.ActionFormMinimal):
    ...
    def create(self):
        self._header = self.add(npyscreen.FixedText, value='{:85} {:45}'.format('Subject',
                                                                                'Sender'),
                                                                                editable=False)
...
class emailDetailForm(npyscreen.ActionForm):
    ...
    def create(self):
        self.from_addr = self.add(npyscreen.TitleFixedText, name="From: ", value='',
                                  editable=False)
        self.subject = self.add(npyscreen.TitleFixedText, name="Subject: ", value='',
                                editable=False)
        self.date = self.add(npyscreen.TitleFixedText, name="Date: ", value='',
                             editable=False)
...
class MyApplication(npyscreen.NPSAppManaged):
    def onStart(self):
        self.login_form = self.addForm('MAIN', liginFrom, name='Email Client')
        self.email_list_form = self.addForm('EMAIL_LIST', emailListFrom, name='Latest Unread Emails')
