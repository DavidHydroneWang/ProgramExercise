...
def Clamp(col):
    """
    Make sure col is between 0 and 255
    """
...
def ColorConversion(Y, Cr, Cb):
    """
    Converts Y, Cr, Cb to RGB color space
    """
    R = Cr * (2 - 2 * 0.299) + Y
    B = Cb * (2 - 2 * 0.114) + Y
...
def GetArray(type, l, length):
    """
    A convience funtion for unpacking an array from bitstream
    """
    s = ""
...
class JPEG:
    ...
    def BuildMatrix(self, st, idx, quant, olddccoeff):
        ...
        dccoeff = DecoderNumber(code, bits) + olddccoeff
