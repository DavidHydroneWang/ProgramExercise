...
class SudokuGrid:
    ...
    def makeMove(self, column, row, number):
        """palce the number at the column (a letter from A to I) and row
    ...
    def _isCompleteSetOfNumbers(self, number):
        """return True if numbers contains the digits 1 through 9."""
        return sorted(number) == list('123456789')

    def isSolved(self):
        """return True if the grid is in a solved state."""
        # check each row.
...
while True:
    ...
    if action.startswith('O'):
        ...
        print('The original grid looked liked this:')
    ...
    if grid.makeMove(column, row, number) is False:
