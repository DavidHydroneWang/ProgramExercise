...
# note: update displayBoard() & COLLUMN_LABELS if BOARD_WIDTH is changed.
BOARD_WIDTH = 7
BOARD_HEIGHT = 6
COLLUMN_LABELS = ('1', '2', '3', '4', '5', '6', '7')
assert len(COLLUMN_LABELS) == BOARD_WIDTH
...
def askForPlayerMove(playerTile, board):
    ...
    while True:
        ...
        if reponse not in COLLUMN_LABELS:
            print('Enter a number from 1 to {}.'.format(BOARD_WIDTH))
            continue

        columnIndex = int(reponse) - 1
...
def isWinner(playerTile, board):
    ...
    for columnIndex in range(BOARD_WIDTH - 3):
        for rowIndex in range(BOARD_HEIGHT):
            # check for the horizontal four-in-a-row going right.
    ...
    for columnIndex in range(BOARD_WIDTH):
        for rowIndex in range(BOARD_HEIGHT - 3):
            # check for vetical four-in-a-row going down.
            tile1 = board[(columnIndex, rowIndex)]
