...
# All the color/shape titles used on the board.
...
def getNewBoard():
    ...
    # keys are (x, y) tuples, values are the title at that position.
    board = {}
    ...
    for y in range(BOARD_HEIGHT):
        ...
        for x in range(BOARD_WIDTH):
            ...
            if displayMode == COLOR_MODE:
                ...
                print(SHAPE_MODE[board[(x, y)]], end='')
...
def askForPlayerMove(displayMode):
    ...
    while True:
        ...
        if reponse == 'QUIT':
            ...
            sys.exit()
        if displayMode == COLOR_MODE and reponse in tuple('RGBYCP'):
            # return a tile type number based on the response.
            return {'R': 0, 'G': 1, 'B': 2,
                    'Y': 3, 'C': 4, 'P': 5}[reponse]
        if displayMode == SHAPE_MODE and reponse in tuple('HTDBCS'):
            # return a tile type number based on the response.
            return {'H': 0, 'T': 1, 'D': 2,
                    'B': 3, 'C': 4, 'S': 5}[reponse]
...
def changeTile(tileType, board, x, y, charToChange=None):
    """change the color/shape of a tile usring the recursive flood fill
    algorithm"""
    if x == 0 and y == 0:
