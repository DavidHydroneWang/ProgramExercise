...
def main():
    ...
    # images should all be the same size and similar exposure
    for i, _ in enumerate(night1_files[:-1]):
        ...
        diff_image1_2 = cv.absdiff(img1, img2)
        cv.imshow('Difference', diff_image1_2)
        cv.waitKey(2000)

        # copy difference image and find and circle brightest pixel.
        temp = diff_image1_2.copy()
        ...
        if transient1 or transient2:
            ...
            cv.putText(img1, night2_files[i], (10, 25),
                       font, 1, (255, 255, 255), 1, cv.LINE_AA)

            blended = cv.addWeighted(img1, 1, diff_image1_2, 1, 0)
