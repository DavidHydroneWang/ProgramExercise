...
class Ship():
    """draw and moves a ship object on an image"""
...
def record_transit(start_image):
    """runs simulation and returns list of itnesity measurements per frame"""
    ship_list = []
    itensity_samples = []
    ...
    for _ in range(NUM_LOOPS):
        ...
        cv.circle(temp_img, (255, 200), STAR_RADIUS, 255, -1)
        for ship in ship_list:
            ship.move_ship(temp_img)
        itensity = temp_img.mean()
        cv.putText(temp_img, 'Mean Intensity = {}'.format(itensity),
                   (5, 300), cv.FONT_HERSHEY_PLAIN, 1, 255)
        cv.imshow('Transit', temp_img)
        itensity_samples.append(itensity)
        cv.waitKey(50)
    cv.destroyAllWindows()
    return itensity_samples


def calc_rel_brightness(image):
    """return list of relative brightness measurements for planetary transit"""
...
def plot_light_curve(rel_brightness):
    """plot curve of relative brightness vs. time"""
