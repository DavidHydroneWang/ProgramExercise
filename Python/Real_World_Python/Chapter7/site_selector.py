...
RECT_HEIGHT_KM = 335  # site rectangle height in kilometers
MAX_ELEV_LIMIT = 55  # intensity values (0-255)
...
RECT_HEIGHT = int(PIXELS_PER_KM * RECT_HEIGHT_KM)
...
class Search():
    ...
    def run_rect_stats(self):
        """define rectanglar search areas and calculate internal stats"""
        ul_x, ul_y = 0, LAT_30_N
        lr_x, lr_y = RECT_WIDTH, LAT_30_N + RECT_HEIGHT
        rect_num = 1
    ...
    def sort_stats(self):
        """sort dictionary by values and create lists of top N keys"""
    ...
    def draw_filtered_rect(self, image, filtered_rect_list):
        """draw rectangle in list on image and return image"""
        img_copy = image.copy()
    ...
    def make_final_display(self):
        ...
        img_color_rects = self.draw_filtered_rect(IMG_COLOR,
                                                  self.high_graded_rects)
...
def main():
    ...
    app.drwa_qc_rects()
    app.sort_stats()
    ptp_img = app.draw_filtered_rect(IMG_GRAY, app.ptp_filtered)
    std_img = app.draw_filtered_rect(IMG_GRAY, app.std_filtered)
